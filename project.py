...
# Load data
# ---------------------------------------------------------------
# Default CSV is loaded from the same folder as this script.
# Optional sidebar uploader lets the user override the default file.
DEFAULT_CSV_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "response.csv"
)
...
if uploaded is not None:
    ...
    if not os.path.exists(DEFAULT_CSV_PATH):
        st.error(
            f"Default file not found at: {DEFAULT_CSV_PATH}\n\n"
            "Place response.csv next to this app.py, or upload a file in the sidebar."
        )
        st.stop()
    df = load_data(DEFAULT_CSV_PATH)
    st.sidebar.info("Using bundled response.csv")
